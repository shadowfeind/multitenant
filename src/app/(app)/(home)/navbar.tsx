...
const NavbarItem = ({href, children, isActive}: NavbarItemProps) => {
    return (
        <Button variant={"outline"} className={cn("bg-transparent hover:bg-transparent rounded-full hover:border-primary border-transparent" +
            "px-3.5 text-lg", isActive && "bg-black text-white hover:bg-black hover:text-white")}
        >
          <Link href={href}>
              {children}
          </Link>
    ...
    return (
        <nav className={"h-20 flex border-b justify-between font-medium bg-white"}>
            ...
            <div className="flex lg:hidden items-center justify-center">
                <Button variant={"ghost"} className={"size-12 border-transparent"} onClick={() => {setSideBarOpen(true)}}>
                    <MenuIcon />
                </Button>
            </div>
        </nav>
